---
title: "Should MCP Servers be Agents in Disguise? [with a Case Study]"
publishedAt: "2026-03-11"
summary: "Most MCP servers are built like vending machines — one button, one item. This post explores why MCP servers should instead be agents in disguise, accepting high-level intents and handling multi-step logic internally."
---
...
The Model Context Protocol suffers from tool sprawl. As Anthropic's own engineering team recently wrote [in this blog](https://www.anthropic.com/engineering/code-execution-with-mcp):
...
![The conventional way of designing MCPs, where every tool is like an API call. However, this results in many round-trip requests that consume latency and tokens.](/blog/mcp-servers/mcp-conventional.webp)

There are solutions, like Claude Code just shipped [MCP Tool Search](https://docs.anthropic.com/en/docs/claude-code/mcp#tool-search) because connecting 5–6 MCP servers was consuming 30–50% of the context window **_before you had even typed a prompt_**. Cloudflare published [Code Mode](https://blog.cloudflare.com/code-mode) and found that having LLMs write code to call MCP tools instead of calling them directly reduced token usage by 81%.
...
![How I refactored Canvas MCP to behave where the orchestrating agent or MCP client calls one tool and the server handles a lot of the low-level logic needed to get the relevant context.](/blog/mcp-servers/mcp-refactored.webp)

## Case Study of Canvas MCP

When I first built the MCP server, I did what everyone does. I mirrored the REST API. Each level of the hierarchy became a tool: _get_courses, get_modules, get_module_items, get_file_url, get_course_assignments,_ etc_._
...
All of the code, raw data and model outputs are available on GitHub at [https://github.com/aryankeluskar/canvas-mcp/tree/master/blog](https://github.com/aryankeluskar/canvas-mcp/tree/master/blog)
...
(2) When latency on trivial queries is critical: adding an internal LLM call adds 500–2000ms, which feels sluggish for "show me course X" queries that were single API calls before. This problem will get less significant as we get more efficient ways of LLM Inference, like [Taalas HC1](https://taalas.com/products/) achieving 16k+ tokens/sec.
